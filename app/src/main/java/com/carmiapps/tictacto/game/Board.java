...
public class Board {
	...
	public boolean isGameEnded() {
		...
		while (!endLoop) {
			...
			for (int m = 0; m < SIZE; m++) {
				if (board[n][m] == Mark.X) {
					...
					if (sum1 == 4) {
						winRow=true;
						endLoop = true;
						gameView.addInstraction("row# " + n + " The winner is X");
					}
				}
				if (getMark(n, m) == Mark.O) {
					...
					if (sum2 == 4) {
						winRow = true;
						endLoop = true;
						gameView.addInstraction("row# " + n + " The winner is O");
					}
				}
			...
			if ( winRow) {
				gameView.addInstraction("row# " + n + " The winner is O");
				gameView.addInstraction("row!" + n + winRow);
				return true;
			}
			...
			for (  n = 0; n < SIZE; n++) {
				if (board[n][m] == Mark.X) {
					...
					if (sum1 == 4) {
						winCOL=true;
						endLoopCol = true;
						gameView.addInstraction("COL# " + m + " The winner is X");
					}
				}
				if (getMark(n, m) == Mark.O) {
					sum2++;
					...
					if (sum2 == 4) {
						winCOL = true;
						endLoopCol = true;
						gameView.addInstraction("COL# " + m + " The winner is O");
					}
				}
		...
		for (int  i = 0; i < this.SIZE; i++) {
			if (board[i][i] == Mark.X) {
				...
				if (sum1 == 4) {
					...
					gameView.addInstraction("Diagonal 1, The winner is X");
				}
			}
			if (board[i][i] == Mark.O) {
				sum2++;
				...
				if (sum2 == 4) {
					windiagonal = true;
					gameView.addInstraction("Diagonal 1, The winner is O");
				}
			}
		...
		for (int i = this.SIZE-1; i >= 0; i--) {
			if (board[this.SIZE-1-i][i] == Mark.X) {
				...
				if (sum1 == 4) {
					...
					gameView.addInstraction("Diagonal 2, The winner is X");
				}
			}
			if (board[i][i] == Mark.O) {
				sum2++;
				...
				if (sum2 == 4) {
					windiagonal1 = true;
					gameView.addInstraction("Diagonal 2, The winner is O");
				}
			}
